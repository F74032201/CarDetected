...
class App:
	...
	def on_init(self):
		...
		self.add_ghost([255,0,0], 320-32, 256-32)
		self.add_ghost([127,0,0], 192-32, 256-32)
	...
	def init_dot(self):
		...
		for i in range(0, int(self.GameWidth/32)):
			for j in range(0, int(self.GameHeight/32)):
				if not([i/2,j/2] in block or [i, j] in [[4, 6], [5, 6], [6, 6], [7, 6], [8, 6],\
				 	[6, 5], [0, 0], [12, 0], [2, 10], [10, 10],	\
	...
	def init_pellet(self):		
		...
		self.pellet.append(Pellet(384+32,0+32))
		self.pellet.append(Pellet(64+32,320+32))
		self.pellet.append(Pellet(320+32,320+32))
		...
		for g in self.ghost:
			if self.game.isCollision_ghost(g.x, g.y, \
				...
				if g.power_mode:
					...
					for i in self.ghost:
						i.on_init()
					self.ghost[1].x = 192-32
					self.ghost[1].y = 256-32
					self.ghost[1].pause_c = 64
	...
	def on_cleanup(self):
		...
		print_mes = ""
		time_mes = "%d分 %d%d秒"%(self.game_time_min,self.game_time_sec10,self.game_time_sec)
		print_mes = print_mes + ("共花時間:" + time_mes + "\n")
		print_mes = print_mes + ("得分: %d\n" %(self.point))
		if len(scoretable) < 10 or (self.point>scoretable[9][1]):
			self.namebox = Toplevel(self.Con.root)
			self.namebox.title('Set Name')

			Label(self.namebox, text="Enter your name:").grid(row=0)

			self.username = Entry(self.namebox)
			self.username.grid(row=0, column=1)
			Button(self.namebox, text='sumit', command=lambda:namebox_control(self)).grid(row=2, column=1, sticky=W, pady=4)

			# mainloop()
		messagebox.showinfo("msg",print_mes)

		scoretable.sort(key=itemgetter(2)) 
		scoretable.sort(key=itemgetter(1), reverse=True) 
		if len(scoretable)>10:
			del scoretable[-1]

		print_mes = ""
		scoreboardtk = Toplevel(self.Con.root)
		scoreboardtk.title('Scoreboard')
		label_frame = []
		for i in range(10):
			print_mes = ""
			if i < len(scoretable):
				print_mes = print_mes + ("%2d."%(i+1)+scoretable[i][0])
				if not hasattr(self, 'player_name'):
					self.player_name = ""
				if scoretable[i][0] == self.player_name:
					label_frame.append(LabelFrame(scoreboardtk, text=print_mes, foreground='red'))
				else:
					print(scoretable[i][0][:],self.player_name[:])
					label_frame.append(LabelFrame(scoreboardtk, text=print_mes, foreground='blue'))
				label_frame[i].pack(fill='x', padx=10, pady=2)
				# aaa = LabelFrame(scoreboardtk, text=print_mes, foreground='blue').pack(fill='x', padx=10, pady=2)
				print_mes = ("%2d分\t"%scoretable[i][1]+scoretable[i][2])
				Label(label_frame[i],text=print_mes).pack(side=LEFT)
			else:
				print_mes = print_mes + ("%2d.\n\n"%(i+1))
				label_frame.append(LabelFrame(scoreboardtk, text=print_mes, foreground='blue'))
				label_frame[i].pack(fill='x', padx=10, pady=2)
				print_mes = ""
				Label(label_frame[i],text=print_mes).pack(side=LEFT)

		Button(scoreboardtk, text='ok', command=lambda:scoreboardtk.quit()).pack()
		ws = scoreboardtk.winfo_screenwidth() # width of the screen
		hs = scoreboardtk.winfo_screenheight() # height of the screen
		# calculate x and y coordinates for the Tk root window
		x = (ws/2) - (700/2)
		y = (hs/2) - (600/2)
		scoreboardtk.geometry('%dx%d+%d+%d' % (300, 430, x, y))
		# messagebox.showinfo("msg", print_mes, fg="darkgreen")

		open('scoreboard.py','w').write('scoretable = '+str(scoretable)+'\n')
		pygame.quit()
		for i in range(0, len(self.wall_v)):
			del self.wall_v[0]
		for i in range(0, len(self.wall_h)):
